...
#boxplot of bmi \# Create a boxplot to visualize the distribution of bmi
in our dataset, it includes the median, quartiles, and potential
outliers of the bmi variable. boxplot.stats(brain_stroke$bmi)$out
...
library(outliers) Outavg_glucose_level =
outlier(brain_stroke\$avg_glucose_level, logical =TRUE)
sum(Outavg_glucose_level) Find_outlier = which(Outavg_glucose_level
==TRUE, arr.ind = TRUE) Outavg_glucose_level Find_outlier

Outbmi = outlier(brain_stroke\$bmi, logical =TRUE) sum(Outbmi)
Find_outlier = which(Outbmi ==TRUE, arr.ind = TRUE) Outbmi Find_outlier

Outage = outlier(brain_stroke\$age, logical =TRUE) sum(Outage)
Find_outlier = which(Outage ==TRUE, arr.ind = TRUE) Outage Find_outlier
#By removing outliers, we improve the overall quality and accuracy of
the dataset. This helps ensure that the data reflects the true
relationships.

#normlization normalize \<- function(x) {return ((x - min(x)) /
(max(x) - min(x)))} dataWithoutNormalization \<- brain_stroke
...
#discretization #we do not need feature discretization because the data
is already well-preprocessed.
...
#delete work_type,ever_married,smoking_status coloumns brain_stroke
\<-data[,!names(data) %in% c("work_type", "smoking_status",
"ever_married")] head(brain_stroke)

#classification \# split 'brain_stroke' dataset into 'trainData' (70% of
the data) for training and 'testData' (30% of the data) for testing.

view(brain_stroke)

str(brain_stroke)

set.seed(1234) ind \<- sample(2, nrow(brain_stroke), replace=TRUE,
prob=c(0.7, 0.3)) trainData \<- brain_stroke[ind == 1,] testData \<-
brain_stroke[ind == 2,]

# Define your formula for classification

install.packages('party') library(party) myFormula \<- stroke \~
gender + age + hypertension + heart_disease + ever_married + work_type +
Residence_type + avg_glucose_level + bmi

# Fit a conditional inference tree to the 'stroke' dataset
